...
const fetchCurrentWeather = ({ authorizationKey, locationName }) => {
  return fetch(
    `${BASE_URL}/O-A0003-001?Authorization=${authorizationKey}&format=JSON&locationName=${locationName}`
  )
...
const fetchWeatherForecast = ({ authorizationKey, cityName }) => {
  return fetch(
    `${BASE_URL}/F-C0032-001?Authorization=${authorizationKey}&locationName=${cityName}`
  )
    .then((response) => response.json())
    .then((data) => {
      const locationData = data.records.location[0];
